...
const initialMemos: Omit<MemoData, 'invoiceId'>[] = [
    {
...
const initDB = (): Promise<IDBDatabase> => {
    return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            initialMemos.forEach(item => memoStore.add({...item, invoiceId: null}));
...
export const getMemosByIds = (memoIds: string[]): Promise<MemoData[]> => {
    return initDB().then(db => {
        return new Promise<MemoData[]>((resolve, reject) => {
            const transaction = db.transaction('memos', 'readonly');
            const store = transaction.objectStore('memos');
            const result: MemoData[] = [];
            let count = memoIds.length;
...
export const getUninvoicedMemosByCustomer = async (customerName: string): Promise<MemoData[]> => {
    const allMemos = await getMemos();
    return allMemos.filter(memo => memo.customers_name === customerName && memo.invoiceId === null);
};
...
export const createSummaryInvoice = async (customerName: string, memoIds: string[], invoiceDate: string): Promise<number> => {
    const db = await initDB();
    const invoiceNumber = await generateNewInvoiceNumber();
    
    return new Promise((resolve, reject) => {
        const transaction = db.transaction(['memos', 'invoices'], 'readwrite');
        const memoStore = transaction.objectStore('memos');
        const invoiceStore = transaction.objectStore('invoices');

        let totalAmount = 0;
        const memosToUpdate: MemoData[] = [];
        let completed = 0;

        memoIds.forEach(memoId => {
            const request = memoStore.get(memoId);
            request.onerror = reject;
            request.onsuccess = () => {
                const memo = request.result as MemoData;
                if (memo) {
                    totalAmount += parseFloat(memo.trips_balance) || 0;
                    memosToUpdate.push(memo);
                }
                completed++;
                if (completed === memoIds.length) {
                    // All memos fetched, create invoice and update memos
                    ...
                    const addInvoiceRequest = invoiceStore.add(newInvoice);
                    addInvoiceRequest.onerror = reject;
                    addInvoiceRequest.onsuccess = () => {
                        const newInvoiceId = addInvoiceRequest.result as number;
                        memosToUpdate.forEach(memo => {
                            memo.invoiceId = newInvoiceId;
                            memoStore.put(memo);
                        });
...
export const deleteInvoice = async (invoiceId: number): Promise<void> => {
    const db = await initDB();
    const invoiceToDelete = await getInvoiceById(invoiceId);
    ...
    return new Promise((resolve, reject) => {
        ...
        deleteRequest.onsuccess = () => {
             invoiceToDelete.memoIds.forEach(memoId => {
                const getReq = memoStore.get(memoId);
                getReq.onsuccess = () => {
                    const memo = getReq.result;
                    if (memo && memo.invoiceId === invoiceId) {
                        memo.invoiceId = null;
                        memoStore.put(memo);
                    }
                }
