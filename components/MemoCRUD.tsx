...
import { getMemos, deleteMemo } from '../services/googleScriptMock';
...
const MemoCRUD: React.FC<MemoCRUDProps> = ({ onEditMemo, onDownloadMemo }) => {
    ...
    const openDeleteConfirmation = (memo: MemoData) => {
        if (memo.invoiceId) {
            addToast('Cannot delete a memo that is already part of an invoice.', 'error');
            return;
        }
                            ...
                            {filteredData.map((memo) => (
                                <tr key={memo.trips_memo_no} className="border-b hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-2">
                                        {memo.invoiceId ? 
                                            <span className="px-2 py-1 text-xs font-semibold text-green-800 bg-green-200 rounded-full">Invoiced</span> :
                                    ...
                                    <td className="px-4 py-2">
                                        <div className="flex space-x-4">
                                            ...
                                            <button onClick={() => openDeleteConfirmation(memo)} className="text-red-600 hover:underline disabled:text-gray-400 disabled:no-underline" disabled={!!memo.invoiceId}>Delete</button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
