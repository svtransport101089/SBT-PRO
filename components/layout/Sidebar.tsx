...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, setCurrentPage, onCreateMemo }) => {
    const mainNavItems = [
        { page: Page.DASHBOARD, label: "Dashboard", icon: <DashboardIcon /> },
    ];
    
    const memoNavItems = [
        { onClick: onCreateMemo, label: "Create Memo", icon: <InvoiceIcon /> },
        { page: Page.MANAGE_MEMOS, label: "Manage Memos", icon: <ManageInvoicesIcon /> },
    ];
    
    const invoiceNavItems = [
        { page: Page.CREATE_INVOICE, label: "Create Summary Invoice", icon: <CreateInvoiceIcon /> },
    ...
    <div className="flex flex-col w-64 bg-gray-800 text-white">
      ...
      <nav className="flex-1 px-2 py-4 space-y-2 overflow-y-auto">
        {mainNavItems.map(item => (
         ...
         <div className="pt-2">
              <h3 className="px-4 text-xs font-semibold uppercase text-gray-400">Memo</h3>
              {memoNavItems.map(item => (
                  <NavLink
                      ...
                      label={item.label}
                      icon={item.icon}
                      isActive={currentPage === item.page || (item.label === 'Create Memo' && currentPage === Page.MEMO && !item.page)}
                      onClick={item.page ? () => setCurrentPage(item.page) : item.onClick}
                  />
              ))}
          </div>
          <div className="pt-2">
              <h3 className="px-4 text-xs font-semibold uppercase text-gray-400">Invoice</h3>
           ...
           <div className="pt-2">
              <h3 className="px-4 text-xs font-semibold uppercase text-gray-400">Data</h3>
